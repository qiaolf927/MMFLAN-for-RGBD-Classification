...
def readJHUIT50Data(filePath, classnum):
    RGBtrain = []
    RGBtest = []
    Dtrain = []
    Dtest = []
    ...
    RGBDtest_label = []
    index = 0
    haselementtrain = False
    haselementtest = False
    for classname in os.listdir(filePath):
        for filename in os.listdir(filePath + '/' + classname):
            image_path = filePath + '/' + classname + '/' + filename
            setjudge = filename[len(classname) + 1]
            modaljudge = filename[-11:]
            if modaljudge == 'rgbcrop.png':
                filename_base = filename[:-11]
                filename_depth = filename_base + 'depthcrop.png'
                if (os.path.exists(image_path)) & (os.path.exists(filePath + '/' + classname + '/' + filename_depth)):
                    img_rgb = Image.open(image_path)
                    img_rgb = img_rgb.resize((112, 112))
                    img_rgb = np.array(img_rgb)
                    img_rgb = np.reshape(img_rgb, (1,) + img_rgb.shape)
                    img_d = Image.open(filePath + '/' + classname + '/' + filename_depth)
...
def readJHUIT50Data_sn(filePath, classnum):
    RGBtrain = []
    RGBtest = []
    Dtrain = []
    Dtest = []
...
if __name__ == '__main__' :
    ...
    RGBtrain, Dtrain, RGBtest, Dtest, RGBDtrain_label, RGBDtest_label = readJHUIT50Data(datasetpath, classnum)
